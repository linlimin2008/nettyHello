...
public class BufferDemo {
    public static void main(String[] args) {
        ...
        //再看数据
        System.out.println(String.format("读取2字节后: capacity容量: %s, position位置: %s, limit限制:%s",byteBuffer.capacity()
                ,byteBuffer.position(),byteBuffer.limit()));
        //继续写入3字节，此时读模式下，limit=3，postion = 2继续写入只能覆盖写入一条数据
        ...
        System.out.println(String.format("最终的情况: capacity容量: %s, position位置: %s, limit限制:%s",byteBuffer.capacity()
                ,byteBuffer.position(),byteBuffer.limit()));
        //rewind() 重置position 为0
        //mark() 标记postion的位置
        //reset() 重置position 为上次mark（）标记的位置
    }
}
